...
export default function MicrosoftConfig() {
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      // Map results back to epStatus
      const next = {};
      (r.data.results || []).forEach((item) => {
        next[item.key] = { ok: item.ok, error: item.error };
      });
      setEpStatus(next);
      ...
      const failed = {};
      ENDPOINTS.forEach((e) => { failed[e.key] = { ok: false, error: 'Request failed' }; });
      setEpStatus(failed);
    } finally {
      setSyncing(false);
    }
  ...
  const okCount = Object.values(epStatus).filter((v) => v !== 'pending' && v.ok).length;
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl overflow-hidden mb-6">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-3 pt-1 flex-wrap">
            ...
            {/* Sync — all 21 endpoints in parallel */}
                ...
                <div key={group}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {eps.map((ep) => {
                      ...
                      const isOk = st && st !== 'pending' && st.ok;
