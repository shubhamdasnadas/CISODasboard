...
export default function MimecastConfig() {
  ...
  useEffect(() => {
    api.get('/mimecast/credentials').then(r => {
  ...
  const handleSaveSync = async () => {
    ...
    try {
      await api.put('/mimecast/credentials', { username: username.trim(), password: password.trim() });
      setStatus('syncing');
      setMsg('Syncing Mimecast data…');
      const r = await api.post('/mimecast/sync');
      setSelectedProvider('emailSecurity', 'Mimecast');
      setMsg((r.data.message || 'Sync complete') + ' — Mimecast is now the active email security tool.');
      setStatus('done');
    } catch (err) {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-2xl">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[var(--foreground)]">Mimecast Configuration</h1>
          <p className="text-sm text-[var(--muted)] mt-1">Configure Mimecast Email Security integration</p>
        </div>
      </div>
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-[var(--card-border)] bg-[var(--muted-bg)] flex items-center gap-3">
          ...
          <h3 className="font-semibold text-[var(--foreground)]">Mimecast</h3>
        </div>
        <div className="p-6 space-y-4">
          {[
            { label: 'Username', val: username, set: setUsername, ph: 'e.g. admin@company.com', type: 'text' },
            { label: 'Password', val: password, set: setPassword, ph: 'API password', type: 'password' },
          ].map(f => (
            <div key={f.label}>
